...
import { buildSemanticIndex, searchSemanticIndex, semanticIndexStatus } from "./semantic-index-engine.mjs";
import { applyDirectorAgentPlan, planDirectorAgent } from "./director-agent-engine.mjs";
import { BACKGROUND_TASK_KINDS, cancelBackgroundTask, listBackgroundTasks, readBackgroundTask, recoverBackgroundTasks, retryBackgroundTask, submitBackgroundTask } from "./task-center-engine.mjs";
...
import { repairProject } from "./project-integrity-engine.mjs";

const server = new McpServer({ name: "cutpilot", version: "10.0.0" });
...
server.tool("cutpilot_create_video", "High-level entry point: create a category project and optionally import an entire local media folder in one call.", { projectPath:z.string(),name:z.string(),starterId:z.enum(["blank","vlog","talking-head","social-short","podcast","wedding","explainer","motion-graphics","product-promo"]).default("vlog"),mediaFolder:z.string().optional(),brief:starterBriefSchema.optional(),format:starterFormatSchema.optional() }, async({projectPath,name,starterId,mediaFolder,brief={},format={}})=>{const project=createProjectFromStarter({name,starterId,brief,format}),imported=[];if(mediaFolder){const folder=resolve(mediaFolder);if(!existsSync(folder))throw new Error(`Media folder not found: ${folder}`);for(const path of walk(folder)){try{imported.push(registerAsset(project,path))}catch(error){if(!/Unsupported media type/.test(error.message))throw error}}}saveProject(projectPath,project);return{content:[{type:"text",text:JSON.stringify({projectPath:resolve(projectPath),projectId:project.id,starter:project.starter,imported:imported.length,next:"Call cutpilot_plan_video with the creative brief"},null,2)}]}});
server.tool("cutpilot_plan_video", "High-level AI entry point: turn an open Chinese or English request into one reviewable whole-video or revision plan. Uses the configured built-in provider, so Codex/Claude decomposition is not required.", {projectPath:z.string(),message:z.string().min(1).max(12000),itemIds:z.array(z.string()).default([]),trackIds:z.array(z.string()).default([]),scope:z.enum(["selection","timeline"]).default("timeline")},async({projectPath,...options})=>{const{project}=loadProject(projectPath);return{content:[{type:"text",text:JSON.stringify(await planIndependentAi(projectPath,project,options),null,2)}]}});
server.tool("cutpilot_apply_plan", "High-level reviewed apply: execute an explicitly approved built-in AI director or revision plan as one snapshot-protected edit.", {projectPath:z.string(),plan:z.object({type:z.literal("cutpilot-independent-ai-plan")}).passthrough(),approved:z.literal(true),replaceTargetTrack:z.boolean().default(true)},async({projectPath,plan,replaceTargetTrack})=>{const{project}=loadProject(projectPath),draft=structuredClone(project);let result;if(plan.intent==="director")result=applyDirectorAgentPlan(draft,plan.plan,{approved:true,targetTrackName:"V2 · AI Director",replaceTargetTrack});else if(plan.intent==="revise")result=applyNaturalLanguageEdit(draft,plan.plan,{approved:true});else throw new Error("Plan is advisory and cannot be applied automatically");createSnapshot(projectPath,"high-level-ai-apply");saveProject(projectPath,draft);return{content:[{type:"text",text:JSON.stringify({applied:true,intent:plan.intent,result,validation:validateProject(draft)},null,2)}]}});
server.tool("cutpilot_finish_video", "High-level finishing gate: audit the current project and, only when approved and release-ready, submit a background MP4 export.", {projectPath:z.string(),outputPath:z.string(),approved:z.boolean().default(false),options:z.record(z.any()).default({})},async({projectPath,outputPath,approved,options})=>{const{project}=loadProject(projectPath),audit=auditCutPilotProject(project);if(!approved||!audit.readyToExport)return{content:[{type:"text",text:JSON.stringify({submitted:false,audit,next:approved?"Resolve every release blocker":"Review the audit, then call again with approved=true"},null,2)}]};const job=submitExportJob(projectPath,{kind:"video",outputPath,options});return{content:[{type:"text",text:JSON.stringify({submitted:true,audit,job},null,2)}]}});
server.tool("configure_cutpilot_ai", "Configure CutPilot's independent AI provider locally. The secret is stored outside project files with user-only permissions.", {provider:z.enum(["openai","anthropic","compatible"]),endpoint:z.string().url(),model:z.string().min(1),apiKey:z.string().optional(),test:z.boolean().default(false)},async({test,...settings})=>{const saved=saveAiSettings(settings),connection=test?await testAiConnection():null;return{content:[{type:"text",text:JSON.stringify({settings:saved,connection},null,2)}]}});
server.tool("repair_cutpilot_project", "Create a safety snapshot, migrate an old project schema, and remove structurally invalid references only after explicit approval.", {projectPath:z.string(),approved:z.literal(true)},async({projectPath})=>{const{project}=loadProject(projectPath),result=repairProject(project);createSnapshot(projectPath,"before-project-repair");saveProject(projectPath,result.project);return{content:[{type:"text",text:JSON.stringify({repaired:result.repaired,changes:result.changes,validation:validateProject(result.project)},null,2)}]}});
...
server.tool("build_semantic_index", "Build and persist a project-wide semantic index covering assets, exact subclip ranges, transcript cues, tags, and local visual intelligence.", { projectPath: z.string() }, async ({ projectPath }) => { const { project } = loadProject(projectPath); return { content: [{ type: "text", text: JSON.stringify(buildSemanticIndex(projectPath, project), null, 2) }] }; });
server.tool("semantic_index_status", "Report whether the persistent semantic asset index exists and whether project changes made it stale.", { projectPath: z.string() }, async ({ projectPath }) => { const { project } = loadProject(projectPath); return { content: [{ type: "text", text: JSON.stringify(semanticIndexStatus(projectPath, project), null, 2) }] }; });
server.tool("search_semantic_assets", "Search local footage semantically across assets, scene subclips, and transcript cues with exact source ranges and repetition avoidance.", { projectPath: z.string(), query: z.string(), limit: z.number().int().min(1).max(50).default(10), assetTypes: z.array(z.enum(["video", "audio", "image", "motion-graphic"])).default([]), kinds: z.array(z.enum(["asset", "subclip", "transcript"])).default([]), avoidAssetIds: z.array(z.string()).default([]), minScore: z.number().default(0) }, async ({ projectPath, ...options }) => { const { project } = loadProject(projectPath); return { content: [{ type: "text", text: JSON.stringify(searchSemanticIndex(projectPath, project, options), null, 2) }] }; });
server.tool("plan_director_agent", "Create a review-first cross-category edit plan by turning captions or a brief into beats and matching varied local shots from the semantic index.", { projectPath: z.string(), brief: z.string().default(""), tone: z.string().default("natural"), pace: z.enum(["calm", "balanced", "energetic"]).default("balanced"), targetDuration: z.number().positive().optional(), requirements: z.array(z.string()).default([]), maxShots: z.number().int().min(1).max(100).default(20) }, async ({ projectPath, ...options }) => { const { project } = loadProject(projectPath); return { content: [{ type: "text", text: JSON.stringify(planDirectorAgent(projectPath, project, options), null, 2) }] }; });
server.tool("apply_director_agent", "Apply an explicitly approved Director Agent plan to a dedicated editable video track and beat markers.", { projectPath: z.string(), plan: z.object({ type: z.literal("cutpilot-director-agent-plan") }).passthrough(), approved: z.boolean().default(false), targetTrackName: z.string().default("V2 · AI Director"), replaceTargetTrack: z.boolean().default(false) }, async ({ projectPath, plan, ...options }) => { const { project } = loadProject(projectPath), result = applyDirectorAgentPlan(project, plan, options); saveProject(projectPath, project); return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] }; });
server.tool("submit_background_task", `Submit a persistent non-blocking CutPilot task. Supported kinds: ${BACKGROUND_TASK_KINDS.join(", ")}.`, { projectPath: z.string(), kind: z.enum(BACKGROUND_TASK_KINDS), options: z.record(z.any()).default({}) }, async ({ projectPath, kind, options }) => ({ content: [{ type: "text", text: JSON.stringify(submitBackgroundTask(projectPath, { kind, options }), null, 2) }] }));
server.tool("read_background_task", "Read one background task's durable status, phase, progress, result, or error.", { projectPath: z.string(), taskId: z.string() }, async ({ projectPath, taskId }) => ({ content: [{ type: "text", text: JSON.stringify(readBackgroundTask(projectPath, taskId), null, 2) }] }));
server.tool("list_background_tasks", "List persistent CutPilot background tasks newest first.", { projectPath: z.string() }, async ({ projectPath }) => ({ content: [{ type: "text", text: JSON.stringify(listBackgroundTasks(projectPath), null, 2) }] }));
server.tool("cancel_background_task", "Cancel a queued or running background task while preserving its history.", { projectPath: z.string(), taskId: z.string() }, async ({ projectPath, taskId }) => ({ content: [{ type: "text", text: JSON.stringify(cancelBackgroundTask(projectPath, taskId), null, 2) }] }));
server.tool("retry_background_task", "Retry a terminal background task as a new traceable task linked to the original.", { projectPath: z.string(), taskId: z.string() }, async ({ projectPath, taskId }) => ({ content: [{ type: "text", text: JSON.stringify(retryBackgroundTask(projectPath, taskId), null, 2) }] }));
server.tool("recover_background_tasks", "Detect abandoned queued/running tasks after a restart and optionally resubmit them safely as linked retries.", { projectPath: z.string(), retry: z.boolean().default(false) }, async ({ projectPath, retry }) => ({ content: [{ type: "text", text: JSON.stringify(recoverBackgroundTasks(projectPath, { retry }), null, 2) }] }));
